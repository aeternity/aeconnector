...
-record(commitment, { delegate::binary(), keyblock::binary(), pogf::binary() }).
-record(block, { hash::binary(), prev_hash::binary(), height::binary(), commitment::list(commitment()) }).
...
get_top_block(Con) ->
  try
    Res = Con:get_top_block(Protocol), true = aehc_parent_block:is_hc_parent_block(Res),
    {ok, Res}
  catch E:R ->
    {error, {E, R}}
  end.
...
get_block_by_hash(Con, Protocol, Hash) ->
  try
    Res = Con:get_block_by_hash(Protocol, Hash), true = aehc_parent_block:is_hc_parent_block(Res),
    {ok, Res}
  catch E:R ->
    {error, {E, R}}
  end.
...
dry_send_tx(Protocol, Con, Delegate, Commitment, PoGF) ->
  try
    ok = Con:dry_send_tx(Protocol, Delegate, Commitment, PoGF)
  catch E:R ->
    {error, {E, R}}
  end.
%%%===================================================================
...
%% Connector will have in part (HC) and out part (particular plugin);
%% By default in part is an mock module;
